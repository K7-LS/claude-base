...
Три режима (разные среды — разные ограничения):
  ...
  --mode markdown    текстовый фолбэк (общий знаменатель Claude/Codex): stdout,
                     при явном --out — в файл. Только канонический формат.

Сквозной принцип skill-development: рендер — кодом, не генерировать HTML заново.
Творческое (что понято → данные) — модель; детерминированное (данные → HTML/MD) — здесь.

ДВА входных формата:

1. КАНОНИЧЕСКИЙ (persisted, <core>/understanding-map.json; признак — поле
   schema_version; схема — ../schemas/understanding-map.schema.json). Строки
   канона — plain text; этот рендер ЭКРАНИРУЕТ их (canonical→view adapter),
   разметка внутри не интерпретируется. Все три режима.

2. LEGACY view-формат (session-local, доверенный вход от Claude; без
   schema_version) — прежний контракт, пропускается как есть, допускает <b>.
   Данные не из доверенного источника сюда не подавать. Режимы widget/standalone.
{
  "title":   "Что я понял: ...",
  ...
  "decisions":[{"ln":"D-1", "title":"...", "detail":"..."}],
  "stamp":   [{"k":"Понял", "v":"Claude", "soft":false}]
}
"""
import argparse
import html
