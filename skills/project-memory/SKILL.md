...
## Смежные файлы ядра (зеркальное решение Claude+Codex, 2026-08-28)

- `<ядро>/understanding-map.json` — канон карты понимания (скилл
  `understanding-map`, запись через его `map_store.py`). Файл ОПЦИОНАЛЕН:
  отсутствие — норма, в признаки валидности ядра НЕ входит; контракт
  bootstrap не меняется (bootstrap карту не создаёт).
- Решения разведены: `<ядро>/DECISIONS.md` — долговечные проектные решения
  со стабильными ID; `_meta/DECISIONS.md` (structured-artifacts) — рабочий
  append-only журнал многофазной задачи; карта понимания — snapshot со
  ссылками на стабильные decision ID, не третий журнал.
- FACTS: новый проект — корневой `FACTS.md`; существующий — единственный
  найденный (`FACTS.md` либо `_meta/FACTS.md`); оба — `FACTS_CONFLICT`
  (выбор канона за пользователем, не объединять).
