...
Правило выбора (зеркальное решение Claude+Codex, 2026-08-28): новый проект
создаётся с корневым `FACTS.md`; в существующем переиспользуй ЕДИНСТВЕННЫЙ
найденный (`FACTS.md` либо `_meta/FACTS.md`); есть оба — `FACTS_CONFLICT`:
останови работу с фактами и попроси пользователя выбрать канон, автоматическое
объединение запрещено. Карта понимания (`understanding-map`) факты НЕ дублирует —
ссылается на FACTS.md через `sources`/`source_ids`.
